...
class BrokerController extends Controller
{
    ...
    public function registerBroker(RegisterBrokerRequest $request)
    {
        try {
            ...
            $broker = DB::transaction(function () use ($request) {
                ...
                $broker = $this->brokerService->registerBroker(
                    $data["trading_name"],
                    $data["broker_type_id"],
                    $data["country_id"],
                );
                ...
                $this->permissionService->createPermission(
                    ...
                    AuthPermission::BROKER,
                    AuthAction::MANAGE,
                );

                //load default challenge categories for the broker
                if (
                    $broker->brokerType->name ==
                    BrokerTypeEnum::PROP_FIRM->value
                ) {
                    $this->challengeService->cloneDefaultChallengesToBroker(
                        $broker->id,
                    );
                }
                ...
                $this->mailService
                    ->to($user->email)
                    ->send(new MagicLinkMail($magicLink));
    ...
    public function show($id, Request $request)
    {
        ...
        $zone = Zone::where("countries", "like", "%$country%")->first()
            ->zone_code;
        if ($tab == "all") {
            //return all options
        } else {
    ...
    public function getBrokerInfo(Request $request, $id)
    {
        try {
            return response()->json(
                $this->brokerService->getBrokerContext($id),
            );
        } catch (\Exception $e) {
            return response()->json(
                [
    ...
    public function getBrokerList(
        ...
        try {
            ...
            $brokers = $this->brokerService->getBrokerList(
                $perPage,
                $orderBy,
                $orderDirection,
                $filters,
    ...
    public function toggleActiveStatus(
        BrokerToggleActiveRequest $request,
        Broker $broker,
    ) {
        try {
            return response()->json(
                $this->brokerService->toggleActiveStatus($broker->id),
            );
        } catch (\Exception $e) {
            return response()->json(
                [
