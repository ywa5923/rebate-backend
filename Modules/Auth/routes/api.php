...
Route::group([], function () {
    ...
    Route::post('/broker-team-user', [
        BrokerTeamUserController::class,
        'registerUserToBrokerDefaultTeam',
    ]);
    Route::put('/broker-team-user/{userId}', [
        BrokerTeamUserController::class,
        'updateBrokerTeamUser',
    ]);
    Route::delete('/broker-team-user/{userId}', [
        BrokerTeamUserController::class,
        'deleteBrokerTeamUser',
    ]);
    Route::get('/broker-default-team/{brokerId}', [
        BrokerTeamUserController::class,
        'getBrokerDefaultTeam',
    ]);
    ...
    Route::middleware(['auth:sanctum', 'superadmin-only'])->get(
        'platform-users/form-config',
        [PlatformUserController::class, 'getFormConfig'],
    );
    Route::middleware(['auth:sanctum', 'superadmin-only'])->apiResource(
        'platform-users',
        PlatformUserController::class,
    );

    Route::middleware(['auth:sanctum', 'superadmin-only'])->patch(
        '/platform-users/toggle-active-status/{platform_user}',
        [PlatformUserController::class, 'toggleActiveStatus'],
    );

    //=================================PLATFORM ADMIN ONLY ROUTES======================================
    //TODO: Establish if platform admins cand add permissions to seo,translators
    Route::middleware('auth:sanctum')->get('/user-permissions', [
        UserPermissionController::class,
        'index',
    ]);
    Route::middleware('auth:sanctum')->delete(
        '/user-permissions/{user_permission}',
        [UserPermissionController::class, 'destroy'],
    );
    Route::middleware('auth:sanctum')->get(
        '/user-permissions/form-config/{permissionType}',
        [UserPermissionController::class, 'getFormConfig'],
    );
    ...
    Route::middleware('auth:sanctum')->patch(
        '/user-permissions/toggle-active-status/{user_permission}',
        [UserPermissionController::class, 'toggleActiveStatus'],
    );

    // Magic link authentication
    Route::post('/login-with-email', [
...
Route::middleware(['auth:sanctum'])->get('/user', function (Request $request) {
    ...
    if ($user instanceof \Modules\Auth\Models\BrokerTeamUser) {
        ...
        $brokerTradingName = $broker->dynamicOptionsValues
        ...
        $userData['permissions'] = $user->resourcePermissions
